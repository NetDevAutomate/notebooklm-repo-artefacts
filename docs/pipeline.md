...
`repo-artefacts pipeline` is the all-in-one command that chains every step — collect, upload, generate, download, publish, verify, and cleanup — into a single invocation. It uses a **stage-based architecture** with state persistence for resumability.
...
The pipeline is implemented as 8 stages, each with pre-check → execute → post-check gates:
...
After each stage, the pipeline saves state to `.pipeline-state.json` in `docs/artefacts/`:
...
With `--resume`, the pipeline loads this state and skips stages that already passed:

```bash
repo-artefacts pipeline /path/to/repo --resume
```

This is useful after quota exhaustion or network failures — you don't need to re-upload or re-generate completed artefacts.
...
sequenceDiagram
    ...
    Note over CLI,NLM: wait_for_completion per artefact<br/>Exponential backoff 2s→10s
...
| `--resume` | Resume from last successful stage | `false` |
